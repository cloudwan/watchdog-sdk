...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return probe.ProbeChangeList(e.Changes)
}
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "probe-query-watcher").
	...
	for {
		stream, err := qw.client.WatchProbes(ctx, &probe_client.WatchProbesRequest{
		...
		if err != nil {
			...
			pending := make([]*probe.ProbeChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetProbeChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(probe.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*probe.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*probe.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*probe.Filter)
		}
		return NewQueryWatcherWithIChan(id, probe_client.NewProbeServiceClient(cc), cfg, ch)
	})
}
