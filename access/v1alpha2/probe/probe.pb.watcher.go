...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[probe.Name]*probe.Probe
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*probe.ProbeChange
}
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &probe.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &probe.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "probe-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[probe.Name]*probe.Probe{}
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[probe.Name]*probe.Probe, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		probe := currentChange.GetCurrent()
		byName[*probe.GetName()] = probe
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			probe := change.GetCurrent()
			byName[*probe.GetName()] = probe
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, probe := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(probe))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*probe.ProbeChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *probe.ProbeChange_Added_:
			newItem := item.Added.GetProbe()
		...
		case *probe.ProbeChange_Modified_:
			updatedItem := item.Modified.GetProbe()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*probe.ProbeChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetProbeName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(probe.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*probe.Probe_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*probe.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(probe_client.NewProbeServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(probe.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*probe.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*probe.ParentReference)
		}
		return params
	})
}
