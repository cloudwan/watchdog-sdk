...
type QueryWatcherParams struct {
	...
	Cursor       *probing_session.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*probing_session.ProbingSessionChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return probing_session.ProbingSessionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client probing_session_client.ProbingSessionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client probing_session_client.ProbingSessionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "probingSession-query-watcher").
	...
	for {
		stream, err := qw.client.WatchProbingSessions(ctx, &probing_session_client.WatchProbingSessionsRequest{
		...
		if err != nil {
			...
			pending := make([]*probing_session.ProbingSessionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetProbingSessionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetProbingSessionChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(probing_session.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*probing_session.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*probing_session.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*probing_session.Filter)
		}
		return NewQueryWatcherWithIChan(id, probing_session_client.NewProbingSessionServiceClient(cc), cfg, ch)
	})
}
