...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *probing_session.ProbingSession_FieldMask
	OrderBy   *probing_session.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *probing_session.ParentReference
	Filter *probing_session.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[probing_session.Name]*probing_session.ProbingSession
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*probing_session.ProbingSessionChange
}

func NewWatcher(client probing_session_client.ProbingSessionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &probing_session.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &probing_session.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "probingSession-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[probing_session.Name]*probing_session.ProbingSession{}
...
type WatcherEventChange struct {
	pre, post *probing_session.ProbingSession
	name      *probing_session.Name
}

func NewAddWatcherEventChange(resource *probing_session.ProbingSession) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *probing_session.ProbingSession) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *probing_session.ProbingSession) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *probing_session.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *probing_session.ProbingSession {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *probing_session.ProbingSession {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *probing_session.ProbingSession {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *probing_session.ProbingSession {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[probing_session.Name]*probing_session.ProbingSession, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		probingSession := currentChange.GetCurrent()
		byName[*probingSession.GetName()] = probingSession
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			probingSession := change.GetCurrent()
			byName[*probingSession.GetName()] = probingSession
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, probingSession := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(probingSession))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[probing_session.Name]*probing_session.ProbingSession{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *probing_session.ProbingSession
		switch item := change.ChangeType.(type) {
		case *probing_session.ProbingSessionChange_Added_:
			currentItem = item.Added.GetProbingSession()
		case *probing_session.ProbingSessionChange_Current_:
			currentItem = item.Current.GetProbingSession()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*probing_session.ProbingSessionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *probing_session.ProbingSessionChange_Added_:
			newItem := item.Added.GetProbingSession()
		...
		case *probing_session.ProbingSessionChange_Modified_:
			updatedItem := item.Modified.GetProbingSession()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*probing_session.ProbingSessionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetProbingSessionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(probing_session.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*probing_session.ProbingSession_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*probing_session.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(probing_session_client.NewProbingSessionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(probing_session.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*probing_session.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*probing_session.ParentReference)
		}
		return params
	})
}
