...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[admin_area.Name]*admin_area.AdminArea
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*admin_area.AdminAreaChange
}

func NewWatcher(client admin_area_client.AdminAreaServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Filter != nil {
			...
			cQuery.Filter = &admin_area.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "adminArea-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[admin_area.Name]*admin_area.AdminArea{}
...
type WatcherEventChange struct {
	pre, post *admin_area.AdminArea
	name      *admin_area.Name
}

func NewAddWatcherEventChange(resource *admin_area.AdminArea) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *admin_area.AdminArea) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *admin_area.AdminArea) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *admin_area.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *admin_area.AdminArea {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *admin_area.AdminArea {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *admin_area.AdminArea {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *admin_area.AdminArea {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[admin_area.Name]*admin_area.AdminArea, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		adminArea := currentChange.GetCurrent()
		byName[*adminArea.GetName()] = adminArea
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			adminArea := change.GetCurrent()
			byName[*adminArea.GetName()] = adminArea
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, adminArea := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(adminArea))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[admin_area.Name]*admin_area.AdminArea{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *admin_area.AdminArea
		switch item := change.ChangeType.(type) {
		case *admin_area.AdminAreaChange_Added_:
			currentItem = item.Added.GetAdminArea()
		case *admin_area.AdminAreaChange_Current_:
			currentItem = item.Current.GetAdminArea()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*admin_area.AdminAreaChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *admin_area.AdminAreaChange_Added_:
			newItem := item.Added.GetAdminArea()
		...
		case *admin_area.AdminAreaChange_Modified_:
			updatedItem := item.Modified.GetAdminArea()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*admin_area.AdminAreaChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetAdminAreaName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(admin_area.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*admin_area.AdminArea_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*admin_area.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(admin_area_client.NewAdminAreaServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(admin_area.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*admin_area.Filter)
		}
		return params
	})
}
