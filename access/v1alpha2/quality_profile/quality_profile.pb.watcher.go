...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[quality_profile.Name]*quality_profile.QualityProfile
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*quality_profile.QualityProfileChange
}

func NewWatcher(client quality_profile_client.QualityProfileServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &quality_profile.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &quality_profile.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "qualityProfile-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[quality_profile.Name]*quality_profile.QualityProfile{}
...
type WatcherEventChange struct {
	pre, post *quality_profile.QualityProfile
	name      *quality_profile.Name
}

func NewAddWatcherEventChange(resource *quality_profile.QualityProfile) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *quality_profile.QualityProfile) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *quality_profile.QualityProfile) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *quality_profile.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *quality_profile.QualityProfile {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *quality_profile.QualityProfile {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *quality_profile.QualityProfile {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *quality_profile.QualityProfile {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[quality_profile.Name]*quality_profile.QualityProfile, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		qualityProfile := currentChange.GetCurrent()
		byName[*qualityProfile.GetName()] = qualityProfile
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			qualityProfile := change.GetCurrent()
			byName[*qualityProfile.GetName()] = qualityProfile
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, qualityProfile := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(qualityProfile))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[quality_profile.Name]*quality_profile.QualityProfile{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *quality_profile.QualityProfile
		switch item := change.ChangeType.(type) {
		case *quality_profile.QualityProfileChange_Added_:
			currentItem = item.Added.GetQualityProfile()
		case *quality_profile.QualityProfileChange_Current_:
			currentItem = item.Current.GetQualityProfile()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*quality_profile.QualityProfileChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *quality_profile.QualityProfileChange_Added_:
			newItem := item.Added.GetQualityProfile()
		...
		case *quality_profile.QualityProfileChange_Modified_:
			updatedItem := item.Modified.GetQualityProfile()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*quality_profile.QualityProfileChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetQualityProfileName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(quality_profile.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		return NewWatcher(quality_profile_client.NewQualityProfileServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(quality_profile.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*quality_profile.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*quality_profile.ParentReference)
		}
		return params
	})
}
