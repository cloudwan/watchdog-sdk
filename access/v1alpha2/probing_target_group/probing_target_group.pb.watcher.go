...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *probing_target_group.ProbingTargetGroup_FieldMask
	OrderBy   *probing_target_group.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *probing_target_group.ParentReference
	Filter *probing_target_group.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[probing_target_group.Name]*probing_target_group.ProbingTargetGroup
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*probing_target_group.ProbingTargetGroupChange
}

func NewWatcher(client probing_target_group_client.ProbingTargetGroupServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &probing_target_group.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &probing_target_group.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "probingTargetGroup-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[probing_target_group.Name]*probing_target_group.ProbingTargetGroup{}
...
type WatcherEventChange struct {
	pre, post *probing_target_group.ProbingTargetGroup
	name      *probing_target_group.Name
}

func NewAddWatcherEventChange(resource *probing_target_group.ProbingTargetGroup) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *probing_target_group.ProbingTargetGroup) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *probing_target_group.ProbingTargetGroup) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *probing_target_group.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *probing_target_group.ProbingTargetGroup {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *probing_target_group.ProbingTargetGroup {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *probing_target_group.ProbingTargetGroup {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *probing_target_group.ProbingTargetGroup {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[probing_target_group.Name]*probing_target_group.ProbingTargetGroup, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		probingTargetGroup := currentChange.GetCurrent()
		byName[*probingTargetGroup.GetName()] = probingTargetGroup
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			probingTargetGroup := change.GetCurrent()
			byName[*probingTargetGroup.GetName()] = probingTargetGroup
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, probingTargetGroup := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(probingTargetGroup))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[probing_target_group.Name]*probing_target_group.ProbingTargetGroup{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *probing_target_group.ProbingTargetGroup
		switch item := change.ChangeType.(type) {
		case *probing_target_group.ProbingTargetGroupChange_Added_:
			currentItem = item.Added.GetProbingTargetGroup()
		case *probing_target_group.ProbingTargetGroupChange_Current_:
			currentItem = item.Current.GetProbingTargetGroup()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*probing_target_group.ProbingTargetGroupChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *probing_target_group.ProbingTargetGroupChange_Added_:
			newItem := item.Added.GetProbingTargetGroup()
		...
		case *probing_target_group.ProbingTargetGroupChange_Modified_:
			updatedItem := item.Modified.GetProbingTargetGroup()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*probing_target_group.ProbingTargetGroupChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetProbingTargetGroupName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(probing_target_group.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*probing_target_group.ProbingTargetGroup_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*probing_target_group.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(probing_target_group_client.NewProbingTargetGroupServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(probing_target_group.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*probing_target_group.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*probing_target_group.ParentReference)
		}
		return params
	})
}
