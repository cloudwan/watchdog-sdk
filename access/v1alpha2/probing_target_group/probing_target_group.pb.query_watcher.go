...
type QueryWatcherParams struct {
	...
	Cursor       *probing_target_group.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*probing_target_group.ProbingTargetGroupChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return probing_target_group.ProbingTargetGroupChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client probing_target_group_client.ProbingTargetGroupServiceClient,
...
func NewQueryWatcherWithIChan(id int, client probing_target_group_client.ProbingTargetGroupServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "probingTargetGroup-query-watcher").
	...
	for {
		stream, err := qw.client.WatchProbingTargetGroups(ctx, &probing_target_group_client.WatchProbingTargetGroupsRequest{
		...
		if err != nil {
			...
			pending := make([]*probing_target_group.ProbingTargetGroupChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetProbingTargetGroupChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetProbingTargetGroupChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(probing_target_group.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*probing_target_group.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*probing_target_group.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*probing_target_group.Filter)
		}
		return NewQueryWatcherWithIChan(id, probing_target_group_client.NewProbingTargetGroupServiceClient(cc), cfg, ch)
	})
}
