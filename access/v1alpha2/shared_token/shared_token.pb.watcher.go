...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *shared_token.SharedToken_FieldMask
	OrderBy   *shared_token.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *shared_token.ParentReference
	Filter *shared_token.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[shared_token.Name]*shared_token.SharedToken
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*shared_token.SharedTokenChange
}

func NewWatcher(client shared_token_client.SharedTokenServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &shared_token.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &shared_token.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "sharedToken-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[shared_token.Name]*shared_token.SharedToken{}
...
type WatcherEventChange struct {
	pre, post *shared_token.SharedToken
	name      *shared_token.Name
}

func NewAddWatcherEventChange(resource *shared_token.SharedToken) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *shared_token.SharedToken) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *shared_token.SharedToken) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *shared_token.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *shared_token.SharedToken {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *shared_token.SharedToken {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *shared_token.SharedToken {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *shared_token.SharedToken {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[shared_token.Name]*shared_token.SharedToken, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		sharedToken := currentChange.GetCurrent()
		byName[*sharedToken.GetName()] = sharedToken
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			sharedToken := change.GetCurrent()
			byName[*sharedToken.GetName()] = sharedToken
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, sharedToken := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(sharedToken))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[shared_token.Name]*shared_token.SharedToken{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *shared_token.SharedToken
		switch item := change.ChangeType.(type) {
		case *shared_token.SharedTokenChange_Added_:
			currentItem = item.Added.GetSharedToken()
		case *shared_token.SharedTokenChange_Current_:
			currentItem = item.Current.GetSharedToken()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*shared_token.SharedTokenChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *shared_token.SharedTokenChange_Added_:
			newItem := item.Added.GetSharedToken()
		...
		case *shared_token.SharedTokenChange_Modified_:
			updatedItem := item.Modified.GetSharedToken()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*shared_token.SharedTokenChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetSharedTokenName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(shared_token.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*shared_token.SharedToken_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*shared_token.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(shared_token_client.NewSharedTokenServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(shared_token.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*shared_token.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*shared_token.ParentReference)
		}
		return params
	})
}
