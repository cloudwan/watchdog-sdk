...
type QueryWatcherParams struct {
	...
	Cursor       *shared_token.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*shared_token.SharedTokenChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return shared_token.SharedTokenChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client shared_token_client.SharedTokenServiceClient,
...
func NewQueryWatcherWithIChan(id int, client shared_token_client.SharedTokenServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "sharedToken-query-watcher").
	...
	for {
		stream, err := qw.client.WatchSharedTokens(ctx, &shared_token_client.WatchSharedTokensRequest{
		...
		if err != nil {
			...
			pending := make([]*shared_token.SharedTokenChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetSharedTokenChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetSharedTokenChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(shared_token.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*shared_token.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*shared_token.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*shared_token.Filter)
		}
		return NewQueryWatcherWithIChan(id, shared_token_client.NewSharedTokenServiceClient(cc), cfg, ch)
	})
}
