...
type QueryWatcherParams struct {
	...
	Cursor       *probe_group.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*probe_group.ProbeGroupChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return probe_group.ProbeGroupChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client probe_group_client.ProbeGroupServiceClient,
...
func NewQueryWatcherWithIChan(id int, client probe_group_client.ProbeGroupServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "probeGroup-query-watcher").
	...
	for {
		stream, err := qw.client.WatchProbeGroups(ctx, &probe_group_client.WatchProbeGroupsRequest{
		...
		if err != nil {
			...
			pending := make([]*probe_group.ProbeGroupChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetProbeGroupChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetProbeGroupChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(probe_group.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*probe_group.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*probe_group.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*probe_group.Filter)
		}
		return NewQueryWatcherWithIChan(id, probe_group_client.NewProbeGroupServiceClient(cc), cfg, ch)
	})
}
