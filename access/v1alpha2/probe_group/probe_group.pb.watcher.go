...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *probe_group.ProbeGroup_FieldMask
	OrderBy   *probe_group.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *probe_group.ParentReference
	Filter *probe_group.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[probe_group.Name]*probe_group.ProbeGroup
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*probe_group.ProbeGroupChange
}

func NewWatcher(client probe_group_client.ProbeGroupServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &probe_group.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &probe_group.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "probeGroup-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[probe_group.Name]*probe_group.ProbeGroup{}
...
type WatcherEventChange struct {
	pre, post *probe_group.ProbeGroup
	name      *probe_group.Name
}

func NewAddWatcherEventChange(resource *probe_group.ProbeGroup) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *probe_group.ProbeGroup) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *probe_group.ProbeGroup) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *probe_group.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *probe_group.ProbeGroup {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *probe_group.ProbeGroup {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *probe_group.ProbeGroup {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *probe_group.ProbeGroup {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[probe_group.Name]*probe_group.ProbeGroup, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		probeGroup := currentChange.GetCurrent()
		byName[*probeGroup.GetName()] = probeGroup
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			probeGroup := change.GetCurrent()
			byName[*probeGroup.GetName()] = probeGroup
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, probeGroup := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(probeGroup))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[probe_group.Name]*probe_group.ProbeGroup{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *probe_group.ProbeGroup
		switch item := change.ChangeType.(type) {
		case *probe_group.ProbeGroupChange_Added_:
			currentItem = item.Added.GetProbeGroup()
		case *probe_group.ProbeGroupChange_Current_:
			currentItem = item.Current.GetProbeGroup()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*probe_group.ProbeGroupChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *probe_group.ProbeGroupChange_Added_:
			newItem := item.Added.GetProbeGroup()
		...
		case *probe_group.ProbeGroupChange_Modified_:
			updatedItem := item.Modified.GetProbeGroup()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*probe_group.ProbeGroupChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetProbeGroupName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(probe_group.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*probe_group.ProbeGroup_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*probe_group.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(probe_group_client.NewProbeGroupServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(probe_group.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*probe_group.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*probe_group.ParentReference)
		}
		return params
	})
}
