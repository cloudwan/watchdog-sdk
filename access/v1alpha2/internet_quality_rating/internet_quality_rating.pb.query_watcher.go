...
type QueryWatcherParams struct {
	...
	Cursor       *internet_quality_rating.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*internet_quality_rating.InternetQualityRatingChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return internet_quality_rating.InternetQualityRatingChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client internet_quality_rating_client.InternetQualityRatingServiceClient,
...
func NewQueryWatcherWithIChan(id int, client internet_quality_rating_client.InternetQualityRatingServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "internetQualityRating-query-watcher").
	...
	for {
		stream, err := qw.client.WatchInternetQualityRatings(ctx, &internet_quality_rating_client.WatchInternetQualityRatingsRequest{
		...
		if err != nil {
			...
			pending := make([]*internet_quality_rating.InternetQualityRatingChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetInternetQualityRatingChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetInternetQualityRatingChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(internet_quality_rating.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*internet_quality_rating.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*internet_quality_rating.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*internet_quality_rating.Filter)
		}
		return NewQueryWatcherWithIChan(id, internet_quality_rating_client.NewInternetQualityRatingServiceClient(cc), cfg, ch)
	})
}
