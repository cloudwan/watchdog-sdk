...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *tag.Tag_FieldMask
	OrderBy   *tag.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *tag.ParentReference
	Filter *tag.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[tag.Name]*tag.Tag
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*tag.TagChange
}

func NewWatcher(client tag_client.TagServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[tag.Name]*tag.Tag{}
...
func NewAddWatcherEventChange(resource *tag.Tag) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *tag.Tag) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *tag.Tag) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *tag.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *tag.Tag {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *tag.Tag {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *tag.Tag {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *tag.Tag {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[tag.Name]*tag.Tag, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		tag := currentChange.GetCurrent()
	...
	for _, change := range src.Changes {
		if change.IsDelete() {
			...
			tag := change.GetCurrent()
			byName[*tag.GetName()] = tag
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, tag := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(tag))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*tag.TagChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *tag.TagChange_Added_:
			newItem := item.Added.GetTag()
		...
		case *tag.TagChange_Modified_:
			updatedItem := item.Modified.GetTag()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*tag.TagChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetTagName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(tag.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*tag.Tag_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*tag.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(tag_client.NewTagServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(tag.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*tag.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*tag.ParentReference)
		}
		return params
	})
}
