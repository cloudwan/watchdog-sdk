...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return tag.TagChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client tag_client.TagServiceClient,
...
func NewQueryWatcherWithIChan(id int, client tag_client.TagServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "tag-query-watcher").
	...
	for {
		stream, err := qw.client.WatchTags(ctx, &tag_client.WatchTagsRequest{
		...
		if err != nil {
			...
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetTagChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(tag.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*tag.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*tag.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*tag.Filter)
		}
		return NewQueryWatcherWithIChan(id, tag_client.NewTagServiceClient(cc), cfg, ch)
	})
}
