...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *probing_config.ProbingConfig_FieldMask
	OrderBy   *probing_config.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *probing_config.ParentReference
	Filter *probing_config.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[probing_config.Name]*probing_config.ProbingConfig
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*probing_config.ProbingConfigChange
}

func NewWatcher(client probing_config_client.ProbingConfigServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &probing_config.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &probing_config.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "probingConfig-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[probing_config.Name]*probing_config.ProbingConfig{}
...
type WatcherEventChange struct {
	pre, post *probing_config.ProbingConfig
	name      *probing_config.Name
}

func NewAddWatcherEventChange(resource *probing_config.ProbingConfig) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *probing_config.ProbingConfig) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *probing_config.ProbingConfig) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *probing_config.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *probing_config.ProbingConfig {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *probing_config.ProbingConfig {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *probing_config.ProbingConfig {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *probing_config.ProbingConfig {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[probing_config.Name]*probing_config.ProbingConfig, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		probingConfig := currentChange.GetCurrent()
		byName[*probingConfig.GetName()] = probingConfig
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			probingConfig := change.GetCurrent()
			byName[*probingConfig.GetName()] = probingConfig
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, probingConfig := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(probingConfig))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[probing_config.Name]*probing_config.ProbingConfig{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *probing_config.ProbingConfig
		switch item := change.ChangeType.(type) {
		case *probing_config.ProbingConfigChange_Added_:
			currentItem = item.Added.GetProbingConfig()
		case *probing_config.ProbingConfigChange_Current_:
			currentItem = item.Current.GetProbingConfig()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*probing_config.ProbingConfigChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *probing_config.ProbingConfigChange_Added_:
			newItem := item.Added.GetProbingConfig()
		...
		case *probing_config.ProbingConfigChange_Modified_:
			updatedItem := item.Modified.GetProbingConfig()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*probing_config.ProbingConfigChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetProbingConfigName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(probing_config.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*probing_config.ProbingConfig_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*probing_config.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(probing_config_client.NewProbingConfigServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(probing_config.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*probing_config.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*probing_config.ParentReference)
		}
		return params
	})
}
