...
type QueryWatcherParams struct {
	...
	Cursor       *probing_config.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*probing_config.ProbingConfigChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return probing_config.ProbingConfigChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client probing_config_client.ProbingConfigServiceClient,
...
func NewQueryWatcherWithIChan(id int, client probing_config_client.ProbingConfigServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "probingConfig-query-watcher").
	...
	for {
		stream, err := qw.client.WatchProbingConfigs(ctx, &probing_config_client.WatchProbingConfigsRequest{
		...
		if err != nil {
			...
			pending := make([]*probing_config.ProbingConfigChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetProbingConfigChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetProbingConfigChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(probing_config.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*probing_config.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*probing_config.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*probing_config.Filter)
		}
		return NewQueryWatcherWithIChan(id, probing_config_client.NewProbingConfigServiceClient(cc), cfg, ch)
	})
}
