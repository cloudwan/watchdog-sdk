...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *agent_software_version.AgentSoftwareVersion_FieldMask
	OrderBy   *agent_software_version.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Filter *agent_software_version.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[agent_software_version.Name]*agent_software_version.AgentSoftwareVersion
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*agent_software_version.AgentSoftwareVersionChange
}

func NewWatcher(client agent_software_version_client.AgentSoftwareVersionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Filter != nil {
			...
			cQuery.Filter = &agent_software_version.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "agentSoftwareVersion-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[agent_software_version.Name]*agent_software_version.AgentSoftwareVersion{}
...
type WatcherEventChange struct {
	pre, post *agent_software_version.AgentSoftwareVersion
	name      *agent_software_version.Name
}

func NewAddWatcherEventChange(resource *agent_software_version.AgentSoftwareVersion) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *agent_software_version.AgentSoftwareVersion) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *agent_software_version.AgentSoftwareVersion) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *agent_software_version.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *agent_software_version.AgentSoftwareVersion {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *agent_software_version.AgentSoftwareVersion {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *agent_software_version.AgentSoftwareVersion {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *agent_software_version.AgentSoftwareVersion {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[agent_software_version.Name]*agent_software_version.AgentSoftwareVersion, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		agentSoftwareVersion := currentChange.GetCurrent()
		byName[*agentSoftwareVersion.GetName()] = agentSoftwareVersion
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			agentSoftwareVersion := change.GetCurrent()
			byName[*agentSoftwareVersion.GetName()] = agentSoftwareVersion
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, agentSoftwareVersion := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(agentSoftwareVersion))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[agent_software_version.Name]*agent_software_version.AgentSoftwareVersion{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *agent_software_version.AgentSoftwareVersion
		switch item := change.ChangeType.(type) {
		case *agent_software_version.AgentSoftwareVersionChange_Added_:
			currentItem = item.Added.GetAgentSoftwareVersion()
		case *agent_software_version.AgentSoftwareVersionChange_Current_:
			currentItem = item.Current.GetAgentSoftwareVersion()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*agent_software_version.AgentSoftwareVersionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *agent_software_version.AgentSoftwareVersionChange_Added_:
			newItem := item.Added.GetAgentSoftwareVersion()
		...
		case *agent_software_version.AgentSoftwareVersionChange_Modified_:
			updatedItem := item.Modified.GetAgentSoftwareVersion()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*agent_software_version.AgentSoftwareVersionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetAgentSoftwareVersionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(agent_software_version.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*agent_software_version.AgentSoftwareVersion_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*agent_software_version.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(agent_software_version_client.NewAgentSoftwareVersionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(agent_software_version.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*agent_software_version.Filter)
		}
		return params
	})
}
