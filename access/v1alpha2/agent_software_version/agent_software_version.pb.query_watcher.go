...
type QueryWatcherParams struct {
	...
	Cursor       *agent_software_version.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*agent_software_version.AgentSoftwareVersionChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return agent_software_version.AgentSoftwareVersionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client agent_software_version_client.AgentSoftwareVersionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client agent_software_version_client.AgentSoftwareVersionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "agentSoftwareVersion-query-watcher").
	...
	for {
		stream, err := qw.client.WatchAgentSoftwareVersions(ctx, &agent_software_version_client.WatchAgentSoftwareVersionsRequest{
		...
		if err != nil {
			...
			pending := make([]*agent_software_version.AgentSoftwareVersionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetAgentSoftwareVersionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetAgentSoftwareVersionChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(agent_software_version.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*agent_software_version.PagerCursor)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*agent_software_version.Filter)
		}
		return NewQueryWatcherWithIChan(id, agent_software_version_client.NewAgentSoftwareVersionServiceClient(cc), cfg, ch)
	})
}
