...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *probing_target.ProbingTarget_FieldMask
	OrderBy   *probing_target.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *probing_target.ParentReference
	Filter *probing_target.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[probing_target.Name]*probing_target.ProbingTarget
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*probing_target.ProbingTargetChange
}

func NewWatcher(client probing_target_client.ProbingTargetServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &probing_target.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &probing_target.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "probingTarget-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[probing_target.Name]*probing_target.ProbingTarget{}
...
type WatcherEventChange struct {
	pre, post *probing_target.ProbingTarget
	name      *probing_target.Name
}

func NewAddWatcherEventChange(resource *probing_target.ProbingTarget) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *probing_target.ProbingTarget) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *probing_target.ProbingTarget) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *probing_target.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *probing_target.ProbingTarget {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *probing_target.ProbingTarget {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *probing_target.ProbingTarget {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *probing_target.ProbingTarget {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[probing_target.Name]*probing_target.ProbingTarget, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		probingTarget := currentChange.GetCurrent()
		byName[*probingTarget.GetName()] = probingTarget
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			probingTarget := change.GetCurrent()
			byName[*probingTarget.GetName()] = probingTarget
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, probingTarget := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(probingTarget))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[probing_target.Name]*probing_target.ProbingTarget{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *probing_target.ProbingTarget
		switch item := change.ChangeType.(type) {
		case *probing_target.ProbingTargetChange_Added_:
			currentItem = item.Added.GetProbingTarget()
		case *probing_target.ProbingTargetChange_Current_:
			currentItem = item.Current.GetProbingTarget()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*probing_target.ProbingTargetChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *probing_target.ProbingTargetChange_Added_:
			newItem := item.Added.GetProbingTarget()
		...
		case *probing_target.ProbingTargetChange_Modified_:
			updatedItem := item.Modified.GetProbingTarget()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*probing_target.ProbingTargetChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetProbingTargetName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(probing_target.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*probing_target.ProbingTarget_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*probing_target.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(probing_target_client.NewProbingTargetServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(probing_target.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*probing_target.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*probing_target.ParentReference)
		}
		return params
	})
}
