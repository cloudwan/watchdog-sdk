...
type QueryWatcherParams struct {
	...
	Cursor       *probing_target.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*probing_target.ProbingTargetChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return probing_target.ProbingTargetChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client probing_target_client.ProbingTargetServiceClient,
...
func NewQueryWatcherWithIChan(id int, client probing_target_client.ProbingTargetServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "probingTarget-query-watcher").
	...
	for {
		stream, err := qw.client.WatchProbingTargets(ctx, &probing_target_client.WatchProbingTargetsRequest{
		...
		if err != nil {
			...
			pending := make([]*probing_target.ProbingTargetChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetProbingTargetChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetProbingTargetChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(probing_target.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*probing_target.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*probing_target.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*probing_target.Filter)
		}
		return NewQueryWatcherWithIChan(id, probing_target_client.NewProbingTargetServiceClient(cc), cfg, ch)
	})
}
