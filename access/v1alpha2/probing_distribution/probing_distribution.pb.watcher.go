...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[probing_distribution.Name]*probing_distribution.ProbingDistribution
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*probing_distribution.ProbingDistributionChange
}

func NewWatcher(client probing_distribution_client.ProbingDistributionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &probing_distribution.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &probing_distribution.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "probingDistribution-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[probing_distribution.Name]*probing_distribution.ProbingDistribution{}
...
type WatcherEventChange struct {
	pre, post *probing_distribution.ProbingDistribution
	name      *probing_distribution.Name
}

func NewAddWatcherEventChange(resource *probing_distribution.ProbingDistribution) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *probing_distribution.ProbingDistribution) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *probing_distribution.ProbingDistribution) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *probing_distribution.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *probing_distribution.ProbingDistribution {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *probing_distribution.ProbingDistribution {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *probing_distribution.ProbingDistribution {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *probing_distribution.ProbingDistribution {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[probing_distribution.Name]*probing_distribution.ProbingDistribution, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		probingDistribution := currentChange.GetCurrent()
		byName[*probingDistribution.GetName()] = probingDistribution
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			probingDistribution := change.GetCurrent()
			byName[*probingDistribution.GetName()] = probingDistribution
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, probingDistribution := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(probingDistribution))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[probing_distribution.Name]*probing_distribution.ProbingDistribution{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *probing_distribution.ProbingDistribution
		switch item := change.ChangeType.(type) {
		case *probing_distribution.ProbingDistributionChange_Added_:
			currentItem = item.Added.GetProbingDistribution()
		case *probing_distribution.ProbingDistributionChange_Current_:
			currentItem = item.Current.GetProbingDistribution()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*probing_distribution.ProbingDistributionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *probing_distribution.ProbingDistributionChange_Added_:
			newItem := item.Added.GetProbingDistribution()
		...
		case *probing_distribution.ProbingDistributionChange_Modified_:
			updatedItem := item.Modified.GetProbingDistribution()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*probing_distribution.ProbingDistributionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetProbingDistributionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(probing_distribution.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*probing_distribution.ProbingDistribution_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*probing_distribution.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(probing_distribution_client.NewProbingDistributionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(probing_distribution.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*probing_distribution.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*probing_distribution.ParentReference)
		}
		return params
	})
}
