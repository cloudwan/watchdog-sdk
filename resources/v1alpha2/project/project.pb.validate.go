...
func (obj *Project) GotenValidate() error {
	...
	if subobj, ok := interface{}(obj.PreferredLocale).(gotenvalidate.Validator); ok {
		if err := subobj.GotenValidate(); err != nil {
			return gotenvalidate.NewValidationError("Project", "preferredLocale", obj.PreferredLocale, "nested object validation failed", err)
		}
	}
...
func (obj *Project_Locale) GotenValidate() error {
	if obj == nil {
		return nil
	}
	{
		rlen := utf8.RuneCountInString(obj.LangugageCode)
		if rlen != 3 {
			return gotenvalidate.NewValidationError("Locale", "langugageCode", obj.LangugageCode, "field must contain exactly 3 characters", nil)
		}
	}
	if cvobj, ok := interface{}(obj).(gotenvalidate.CustomValidator); ok {
		return cvobj.GotenCustomValidate()
